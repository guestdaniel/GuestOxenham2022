...
# Load simulations
sims = list.files('supfigure_levels', pattern='fdls.csv')
fdls = data.frame()
for (sim in 1:length(sims)) {
	# Import each simulation CSV
	temp = read.csv(file.path('supfigure_levels', sims[sim]))
...
fdls %>% 
	...
	facet_grid(. ~ model)
# Save plot to disk
ggsave('plots/supfigure_levels_fdls.png', width=5, height=3)
