...
for (sim in 1:length(sims)) {
	...
	temp = read.csv(file.path(root_directory, '/supfigure_levels/', sims[sim]))
...
f0dls %>% 
	...
	ggplot(aes(x=F0, y=threshold/(F0)*100, color=as.numeric(as.character(nominal_level)), shape=decoding_type, group=interaction(nominal_level, decoding_type))) + 
	# Geoms
