...
for (sim in 1:length(sims)) {
	f0dls = bind_rows(f0dls, read.csv(file.path(root_directory, 'figure5', sims[sim])))
}
f0dls$threshold = f0dls$result
f0dls$nominal_level = factor(f0dls$nominal_level)
f0dls$roving_type = factor(f0dls$roving_type, levels=c("none", "level", "phase"), labels=c("None", "Level Roved", "Phase Randomized"))
f0dls$model = factor(f0dls$model, levels=c("Heinz2001", "Zilany2014", "Verhulst2018"), labels=c("Heinz et al. (2001)", "Zilany et al. (2014)", "Verhulst et al. (2018)"))
...
f0dls %>% 
	...
	# Facets
	facet_grid(. ~ model)
ggsave(file.path(root_directory, 'plots', 'fig5a.png'), width=6, height=2.5)
