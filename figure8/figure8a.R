...
# Load simulations
sims = list.files(file.path(root_directory, '/figure8/'), pattern='.csv')
f0dls = data.frame()
for (sim in 1:length(sims)) {
	# Import each simulation CSV
	temp = read.csv(file.path(root_directory, '/figure8/', sims[sim]))
...
f0dls %>% 
	...
	# Facets
	facet_grid(. ~ model)
ggsave(file.path('plots', 'fig8a.png'), width=8, height=3)
