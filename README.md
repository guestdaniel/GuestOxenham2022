...
This repository contains all the code necessary to completely replicate the figures and analyses found in Guest and Oxenham (2021), in preparation. The codebase is a mixture of Python and R, with Python mostly being used to conduct neural simulations and R mostly being used to analyze behavioral data and create figures. A Docker image is available in which a single script `run.sh` can be run to replicate all of the figures in the paper. This is our recommended solution for running the code in this repository.
...
│   ├── figure6a.py          # Python code to generate run simulations featured in Figure 6
│   ├── figure6a.R           # R code to plot results from figure6a.py
│   └── ...      
├── figure7                  # Code for Figure 6, similar in structure to /figure6
...
Figure 4 plots behavioral results from Experiment 2. The entire figure is generated by a single `.R` script. Corresponding mixed effects models are available in `nofigure/behavioral_data_analysis`.
